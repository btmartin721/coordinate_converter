...
def read_coordinates(lst):
    ...
    dms_regex = re.compile(ur"^\s*([A-Za-z]?)\s*(-?)(\d{1,3})\u00b0?\s+(\d{1,2})'?\s+(\d+)\"?\.?(\d*)\"?\s*([A-Za-z]?)$", re.UNICODE)
    ddm_regex = re.compile(ur"^\s*([A-Za-z]?)\s*(-?)(\d{1,3})\u00b0?\s+(\d{1,2})'?\.{1}(\d+)\"?\s*([A-Za-z]?)$", re.UNICODE)
    ...
    for item in lst:
        ...
        if dms_match:
            ...
            temp_dash = dms_match.group(2)
            temp_deg = dms_match.group(3)
            temp_min = dms_match.group(4)
            temp_sec = dms_match.group(5)
            temp_deci = dms_match.group(6)
            
                       
            dir_sign = check_direction(1, 7, dms_match, line_number)
...
def check_direction(group_number1, group_number2, match, line):
    ...
    if match.group(group_number1) is "N" or match.group(group_number1) is "E":
        ...
        print("Warning: No cardinal direction or negative sign defined on line(s) " + str(line) + "; default = + (N or E)")
        sign = r"positive"
    else:
